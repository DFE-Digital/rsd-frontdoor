...
resource "azurerm_cdn_frontdoor_origin_group" "rsd" {
  for_each = local.enable_frontdoor ? merge([
    for profile_name, profile_origins in local.frontdoor_profiles : {
      for origin_name, origin_values in profile_origins : origin_name => merge({ profile_name = profile_name }, origin_values)
    }
  ]...) : {}

  name                     = "${local.environment}-${each.value["profile_name"]}-${each.key}"
...
resource "azurerm_cdn_frontdoor_origin" "rsd" {
  ...
  dynamic "private_link" {
    ...
    content {
      request_message        = "Request access for Private Link from RSD FrontDoor CDN Origin (${local.environment}-${each.value["profile_name"]}-${each.key})"
...
resource "azurerm_cdn_frontdoor_endpoint" "rsd" {
  ...
  name                     = substr("${local.environment}-${each.value["profile_name"]}-${each.key}", 0, 46)
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.rsd[each.value["profile_name"]].id
...
resource "azurerm_cdn_frontdoor_route" "rsd" {
  ...
  cdn_frontdoor_rule_set_ids = concat(compact([
    local.enable_frontdoor_vdp_redirects ? azurerm_cdn_frontdoor_rule_set.vdp[each.value["profile_name"]].id : null,
    each.value.enable_security_headers ? azurerm_cdn_frontdoor_rule_set.security[each.value["profile_name"]].id : null,
