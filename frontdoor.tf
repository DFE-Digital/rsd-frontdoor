...
resource "azurerm_cdn_frontdoor_route" "rsd" {
  ...
  cdn_frontdoor_rule_set_ids = concat(compact([
    ...
    each.key == "complete-ruby" && local.enable_custom_reroute_ruleset && !local.enable_custom_reroute_reversal ? azurerm_cdn_frontdoor_rule_set.complete_dotnet_ruby_migration[0].id : null,
    each.key == "complete-dotnet" && local.enable_custom_reroute_ruleset && local.enable_custom_reroute_reversal ? azurerm_cdn_frontdoor_rule_set.complete_dotnet_ruby_migration[0].id : null,
