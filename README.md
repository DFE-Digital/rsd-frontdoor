...
| <a name="enable_custom_reroute_reversal"></a> [enable\_custom\_reroute\_reversal](#input\_enable\_custom\_reroute\_reversal) | Toggle on/off the reversal of the ruby-dotnet traffic forwarding. A temporary flag while dev, test and prod are de-synced. Once test/prod environments follow dev (dotnet traffic to ruby, rather than ruby to dotnet) we can remove this flag | `bool` | `false` | no |
